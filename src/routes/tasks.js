...
router.post('/project/:projectId', authenticateAny, (req, res) => {
    try {
        ...
        const result = db.prepare(`
            ...
            VALUES (?, ?, ?, ?, ?, ?, ?)
        `).run(projectId, title, description || null, taskPriority, assigned_to || null, due_date || null, taskStatus);
...
router.patch('/:id', authenticateAny, (req, res) => {
    try {
        ...
        // Allowed fields
        const allowedFields = ['title', 'description', 'priority', 'assigned_to', 'due_date', 'status'];
        ...
        // Log activity
        logActivity(db, req.actor.name, req.actor.type, 'update', 'task', parseInt(id), {
